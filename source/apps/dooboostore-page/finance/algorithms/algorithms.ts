...
async function load5MinuteCharts() {
  console.log('📊 Starting 5-minute chart data collection...');
  ...
  // Check which intervals to try (shortest first)
  const intervals = ['1m', '2m', '5m'];
  ...
  for (let batchStart = 0; batchStart < symbols.length; batchStart += BATCH_SIZE) {
    ...
    const batchPromises = batch.map(async (symbol, index) => {
      ...
      for (const interval of intervals) {
        const intervalDir = join(CHART_DIR, interval);
      ...
      try {
        ...
        // Set appropriate date range based on interval
        const intervalDays: Record<string, number> = {
          '1m': 5,   // 5 days for 1-minute data
          '2m': 30,  // 30 days for 2-minute data
          '5m': 30   // 30 days for 5-minute data
        };
        
        const daysToFetch = intervalDays['5m'] || 30; // Default to 30 days
        ...
        for (const interval of intervals) {
          try {
            console.log(`  [${symbol}] Trying interval: ${interval}`);
            
            // Adjust date range based on interval
            const intervalDays: Record<string, number> = {
              '1m': 5,   // 5 days for 1-minute
              '2m': 30,  // 30 days for 2-minute
              '5m': 30   // 30 days for 5-minute
            };
            
            const adjustedStartDate = new Date();
            adjustedStartDate.setDate(adjustedStartDate.getDate() - (intervalDays[interval] || 30));
            
            chartData = await yahooService.chart(symbol, {
              period1: adjustedStartDate,
              period2: endDate,
              interval: interval
            });
            ...
            if (chartData && chartData.quotes && chartData.quotes.length > 0) {
              ...
              console.log(`  ✅ [${symbol}] Success with ${interval} (${chartData.quotes.length} data points)`);
              break;
            }
          } catch (error) {
            const errorMsg = (error as Error).message;
            console.log(`  ⚠️ [${symbol}] Failed with ${interval}: ${errorMsg}`);
            
            if (errorMsg.includes('Too Many Requests') || errorMsg.includes('429')) {
              // Wait a bit and try next interval
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ 5-minute chart data collection completed!');
  console.log(`📊 Summary:`);
...
const algorithms = async () => {
  const startDate = new Date('2025-12-30T09:00:00+09:00'); // KST 기준
  const endDate = new Date('2025-12-30T15:30:00+09:00'); // KST 기준
  const minute = 1; // 1 minute interval
  ...
  // 골든크로스 / 데드크로스 설정
  const goldenCross = { from: 5, to: 20, under: [50], minSlope: 0.0005 }; // 5일선이 20일선을 상향 돌파, 5일선이 50일선보다 위, 5일선 기울기 0.02 이상 (2% 변화, 0~1 범위에서 1=100%)
  ...
  const config = {
    ...
    buy: {
      ...
      stockSize: 10,  // 각 종목당 10주씩 매수 (수량 증가)
  ...
  groups.forEach(group => {
    group.symbols.filter(it => !symbols.has(it)).forEach(symbol => {
      const chartPath = join(CHART_DIR, '1m', `${symbol}.json`);
      if (existsSync(chartPath)) {
        ...
        console.log(`Loaded 1m chart for ${symbol}, ${chartData.quotes.length} data points`);
      } else {
        console.log(`1m chart not found for ${symbol}, skipping`);
      }
    });
  })
            ...
            if (prevFromMAGolden.value <= prevToMAGolden.value && currFromMAGolden.value > currToMAGolden.value) {
              ...
              group.symbols.forEach(symbol => {
                ...
                if (fromMA && toMA && fromMA.value > toMA.value) {
                  ...
                  // 이미 보유 중이면 스킵
                  if (account.holdings.has(symbol)) {
                    console.log(`        ⚠️  Already holding ${symbol}, skipping`);
                    return;
                  }
