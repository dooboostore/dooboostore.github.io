...
const algorithms = async (dataPlan: DataPlan, user: User) => {
  ...
  const allMAPeriods = Array.from(
    new Set([...maPeriods, goldenCross.from, goldenCross.to, deadCross.from, deadCross.to])
  ).sort((a, b) => a - b);
  ...
  const buyStock = (
    ...
    // 거래 중단 상태 체크
    if (config.features.consecutiveLossProtection && account.tradingPaused) {
    ...
    if (config.features.bollingerBandsFilter && bollingerBands) {
      if (bollingerBands.percentB < config.buy.minBollingerPercentB) {
        console.log(
          `    ⚠️  Price too close to lower band (%B: ${(bollingerBands.percentB * 100).toFixed(1)}%), skipping buy`
        );
        return;
      }
      if (bollingerBands.percentB > config.buy.maxBollingerPercentB) {
        console.log(
          `    ⚠️  Price too high in band (%B: ${(bollingerBands.percentB * 100).toFixed(1)}%), skipping buy`
        );
        return;
      }
    }
    ...
    const quotesUntilNow = symbolData.quotes.filter(
      q => q.date.getTime() <= currentTime.getTime() && q.close !== null && q.close !== undefined
    );
      ...
      if (isGoldenCrossEntry) {
        ...
        if (symbolTimeSeries && symbolTimeSeries.length >= 2) {
          ...
          if (prevFromMA && fromMA.slope <= prevFromMA.slope) {
            console.log(
              `    ⚠️  Slope not increasing (${fromMA.slope.toFixed(2)}% vs ${prevFromMA.slope.toFixed(2)}%), skipping pyramiding`
            );
            return;
          }
        }
    ...
    if (holding && config.features.pyramiding) {
      ...
      console.log(
        `    📊 Pyramiding quantity: ${quantity}주 (investment: ${nextInvestment.toLocaleString()}원, current holding: ${currentHolding}주)`
      );
    } else {
      console.log(
        `    💰 Investment: ${investmentAmount.toLocaleString()}원 (${(config.buy.stockRate * 100).toFixed(1)}% of balance)`
      );
    }

    if (quantity === 0) {
      console.log(
        `    ⚠️  Insufficient balance for even 1 share (price: ${price.toLocaleString()}원, available: ${investmentAmount.toLocaleString()}원)`
      );
      return;
    }
    ...
    if (account.balance < total) {
      console.log(
        `    ❌ Insufficient balance for ${symbol} (need: ${total.toLocaleString()}, have: ${account.balance.toLocaleString()})`
      );
      return;
    }
    ...
    console.log(
      `    ✅ BUY ${symbol}: ${quantity}주 @ ${price.toLocaleString()}원${pyramidingLabel} (isPyramiding: ${isPyramiding}, group: ${group.label}, slope: ${fromMA.slope.toFixed(2)}%, vol: ${volumeStrength.toFixed(1)}%, rsi: ${rsi?.toFixed(1) || 'N/A'}, macd: ${macd?.histogram.toFixed(4) || 'N/A'}, bb: ${bollingerBands ? (bollingerBands.percentB * 100).toFixed(1) + '%' : 'N/A'})`
    );
  ...
  const sellStock = (
    ...
    if (config.features.consecutiveLossProtection) {
      if (profit < 0) {
        ...
        if (account.consecutiveLosses >= config.riskManagement.maxConsecutiveLosses) {
          account.tradingPaused = true;
          console.log(`    🚨 Trading PAUSED due to ${account.consecutiveLosses} consecutive losses`);
        }
      } else {
        account.consecutiveLosses = 0; // 수익 나면 리셋
    ...
    const emoji =
      reason === 'STOP_LOSS' ? '🛑' : reason === 'TAKE_PROFIT' ? '🎯' : reason === 'TRAILING_STOP' ? '📉' : '☠️';
    const remainingInfo = remainingQty > 0 ? ` (남은 수량: ${remainingQty}주)` : '';
    console.log(
      `    ${emoji} SELL ${symbol} (${reason}): ${quantity}주 @ ${price.toLocaleString()}원 (profit: ${profit >= 0 ? '+' : ''}${profit.toLocaleString()}원 / ${profitRate >= 0 ? '+' : ''}${profitRate.toFixed(2)}%)${remainingInfo}`
    );
    console.log(`    💵 Balance: ${account.balance.toLocaleString()}원`);
  };
  ...
  const checkStopLossAndTakeProfit = (currentTime: Date): Set<string> => {
    const toSell: {
      ...
      reason: string;
      price: number;
      holding: { quantity: number; avgPrice: number; maxPrice: number; buyTime: Date };
    }[] = [];
    ...
    account.holdings.forEach((holding, symbol) => {
      ...
      if (symbol === '066970.KS' && profitRate > 0.02) {
        ...
        console.log(
          `  [DEBUG] ${symbol} profit check: ${(profitRate * 100).toFixed(2)}% (state: ${currentState}, takeProfit enabled: ${config.features.takeProfit})`
        );
      }
      ...
      // 익절 체크 (기능 활성화 시에만) - 항상 체크 (손절이 없을 때만)
        ...
        if (symbol === '066970.KS') {
          console.log(
            `  [DEBUG] ${symbol} TAKE PROFIT triggered! ${(profitRate * 100).toFixed(2)}% >= ${(config.sell.takeProfit * 100).toFixed(2)}%`
          );
        }
    ...
    if (toSell.length > 0) {
      ...
      toSell.forEach(item => {
        ...
        console.log(
          `  ${emoji} ${item.reason} ${item.symbol}: ${quantity}주 @ ${price.toLocaleString()}원 (${profitRate >= 0 ? '+' : ''}${profitRate.toFixed(2)}%, profit: ${profit >= 0 ? '+' : ''}${profit.toLocaleString()}원)${maxPriceInfo}`
        );
      });
    ...
    group.symbols
      ...
      .forEach(symbol => {
        ...
        if (existsSync(chartPath)) {
          ...
          const quotes = allQuotes.filter((quote, index) => {
            if (index === 0) return true;
            const prevQuote = allQuotes[index - 1];
            // close와 volume이 같으면 중복으로 판단
          ...
          console.log(
            `Loaded ${dataPlan.interval} chart for ${symbol}, ${allQuotes.length} -> ${quotes.length} data points (duplicates removed)`
          );
        } else {
  ...
  while (currentTime <= dataEndDate) {
    // 데이터 종료 시점까지 (차트 전체 기간)
    const isAlgoActive =
      currentTime.getTime() >= algoStartDate.getTime() && currentTime.getTime() <= algoEndDate.getTime(); // 거래 활성화 여부
    ...
    groups.forEach(group => {
      ...
      group.symbols.forEach(symbol => {
        ...
        const quotesUntilNow = quotes.filter(
          q => q.date.getTime() <= currentTime.getTime() && q.close !== null && q.close !== undefined
        );
        ...
        if (previousQuotes.length > 0 && currentVolume > 0) {
          ...
          if (validPreviousVolumes.length > 0) {
            const avgVolume =
              validPreviousVolumes.reduce((sum, q) => sum + (q.volume || 0), 0) / validPreviousVolumes.length;
          ...
          if (currFromMADead && currToMADead) {
            if (currFromMADead.value < currToMADead.value) {
              ...
              if (prevState !== 'DEAD') {
                ...
                console.log(
                  `  ☠️  DEAD CROSS [${timeStr}]: ${symbol} - MA${deadCross.from} (${currFromMADead.value.toFixed(2)}) < MA${deadCross.to} (${currToMADead.value.toFixed(2)})`
                );
                ...
                if (deadCross.below && deadCross.below.length > 0) {
                  for (const belowPeriod of deadCross.below) {
                    ...
                    if (belowMA && currFromMADead.value < belowMA.value) {
                      ...
                      console.log(
                        `    🚨 BELOW THRESHOLD: MA${deadCross.from} (${currFromMADead.value.toFixed(2)}) < MA${belowPeriod} (${belowMA.value.toFixed(2)}) - FULL SELL!`
                      );
                      break;
                    }
                  }
                  ...
                  for (const belowPeriod of deadCross.below) {
                    ...
                    if (belowMA && currFromMADead.value < belowMA.value) {
                      ...
                      if (prevFromMA && prevBelowMA && prevFromMA.value >= prevBelowMA.value) {
                        ...
                        const timeStr = `${currentTime.getHours()}:${currentTime.getMinutes().toString().padStart(2, '0')}`;
                        console.log(
                          `    🚨 BELOW THRESHOLD [${timeStr}]: MA${deadCross.from} (${currFromMADead.value.toFixed(2)}) dropped below MA${belowPeriod} (${belowMA.value.toFixed(2)}) - FULL SELL!`
                        );
                        sellStock(
                          symbol,
                          currentTime,
                ...
                if (
                  isAlgoActive &&
                  !belowConditionMet &&
                  config.features.deadCrossAdditionalSell &&
                    ...
                    if (symbolData) {
                      ...
                      if (currentQuote && currentQuote.close) {
                        ...
                        if (priceDecline >= config.sell.additionalSellThreshold) {
                          const timeStr = `${currentTime.getHours()}:${currentTime.getMinutes().toString().padStart(2, '0')}`;
                          console.log(
                            `    📉 Additional decline detected [${timeStr}]: ${symbol} - ${(priceDecline * 100).toFixed(2)}% down from last sell (${lastSellPrice.toLocaleString()} → ${currentPrice.toLocaleString()})`
                          );
            ...
            // 2. 골든크로스 상태 체크 (데드크로스가 아닐 때만)
              ...
              if (prevState !== 'GOLDEN') {
                ...
                const timeStr = `${currentTime.getHours()}:${currentTime.getMinutes().toString().padStart(2, '0')}`;
                console.log(
                  `  🌟 GOLDEN CROSS [${timeStr}]: ${symbol} - MA${goldenCross.from} (${currFromMAGolden.value.toFixed(2)}) > MA${goldenCross.to} (${currToMAGolden.value.toFixed(2)})`
                );

                // 골든크로스 발생 = G 마크 표시 (순수 골든크로스, 추가 조건 없음)
                isSymbolGoldenCross = true;
                console.log(`    ✅ Golden cross detected - marking on chart`);
              ...
              if (canBuy) {
                ...
                if (goldenCross.under && goldenCross.under.length > 0) {
                  for (const underPeriod of goldenCross.under) {
                    ...
                    if (underMA && currFromMAGolden.value <= underMA.value) {
                      ...
                      console.log(
                        `    ⚠️  [BUY] Under condition failed: MA${goldenCross.from} (${currFromMAGolden.value.toFixed(2)}) <= MA${underPeriod} (${underMA.value.toFixed(2)})`
                      );
                      break;
                    }
                  }
                ...
                if (
                  goldenCross.minSlope !== undefined &&
                  goldenCross.minSlope > 0 &&
                  currFromMAGolden.slope < goldenCross.minSlope
                ) {
                  slopeConditionMet = false;
                  console.log(
                    `    ⚠️  [BUY] Slope condition failed: MA${goldenCross.from} slope (${currFromMAGolden.slope.toFixed(6)}) < minSlope (${goldenCross.minSlope})`
                  );
                }
                  ...
                  if (isGoldenCrossEntry) {
                    ...
                    if (!account.holdings.has(symbol) && currentState === 'GOLDEN') {
                      ...
                      if (soldSymbolsThisTime.has(symbol)) {
                        const timeStr = `${currentTime.getHours()}:${currentTime.getMinutes().toString().padStart(2, '0')}`;
                        console.log(
                          `  ⏸️  SKIP RE-BUY [${timeStr}]: ${symbol} - Sold in this time point, will re-buy next time if still in golden cross`
                        );
                      } else {
                        shouldBuy = true;
                        const timeStr = `${currentTime.getHours()}:${currentTime.getMinutes().toString().padStart(2, '0')}`;
                        ...
                        if (!hasFirstBuyInCycle) {
                          console.log(
                            `  🔄 RE-BUY OPPORTUNITY [${timeStr}]: ${symbol} - First buy in golden cross cycle (label: "b")`
                          );
                        } else {
                          console.log(
                            `  🔄 RE-BUY OPPORTUNITY [${timeStr}]: ${symbol} - No holdings in golden cross state (RE-BUY, label: "!b")`
                          );
                        }
                      }
                    }
                    // 케이스 2: 이전에 조건 불만족이었다가 지금 만족 (피라미딩)
                      ...
                      if (!prevUnderConditionMet || !prevSlopeConditionMet) {
                        ...
                        const timeStr = `${currentTime.getHours()}:${currentTime.getMinutes().toString().padStart(2, '0')}`;
                        console.log(
                          `  ✨ CONDITIONS MET [${timeStr}]: ${symbol} - Conditions satisfied while in golden cross state`
                        );
                    ...
                    if (boughtSymbolsThisTime.has(symbol)) {
                      ...
                      if (symbolGroup) {
                        ...
                        if (config.features.onlySymbolGoldenCross) {
                          console.log(
                            `    ✅ onlySymbolGoldenCross enabled, attempting buy without group check (isReBuy: ${isReBuy}, hasFirstBuy: ${hasFirstBuyInCycle}, isGoldenCrossEntry: ${isGoldenCrossEntry})`
                          );
                          bought = buyStock(
                            symbol,
                            symbolGroup,
                          ...
                          if (buyableGroups.has(symbolGroup.group)) {
                            console.log(
                              `    ✅ Group ${symbolGroup.label} is in buyable list, attempting buy (isReBuy: ${isReBuy}, hasFirstBuy: ${hasFirstBuyInCycle}, isGoldenCrossEntry: ${isGoldenCrossEntry})`
                            );
                            bought = buyStock(
                              symbol,
                              symbolGroup,
        ...
        // 심볼별 시계열 데이터 저장 (이전과 같은 데이터면 스킵 - 장 마감 중복 제거)
        const symbolTS = symbolTimeSeriesMap.get(symbol);
        const lastEntry = symbolTS && symbolTS.length > 0 ? symbolTS[symbolTS.length - 1] : null;
        const isDuplicate =
          lastEntry && lastEntry.avgChangeRate === changeRate && lastEntry.avgVolumeStrength === volumeStrength;

        if (!isDuplicate) {
          if (isSymbolGoldenCross) {
            console.log(
              `  📊 [DEBUG] ${symbol}: Saving goldenCross=true to timeSeries at ${currentTime.toISOString()}`
            );
          }
        ...
        console.log(
          `  ${symbol}: ${changeRate >= 0 ? '+' : ''}${changeRate.toFixed(2)}% | Vol: ${volumeStrength >= 0 ? '+' : ''}${volumeStrength.toFixed(1)}% (${currentVolume.toLocaleString()})`
        );
      });
      ...
      if (validSymbolCount > 0) {
        ...
        console.log(
          `  ✨ Group Average: ${avgChangeRate >= 0 ? '+' : ''}${avgChangeRate.toFixed(2)}% | Vol: ${avgVolumeStrength >= 0 ? '+' : ''}${avgVolumeStrength.toFixed(1)}%`
        );
        ...
        if (isAlgoActive && prevGroupTimeSeries) {
          ...
          if (prevFromMAGolden && prevToMAGolden && currFromMAGolden && currToMAGolden) {
            ...
            if (prevFromMAGolden.value <= prevToMAGolden.value && currFromMAGolden.value > currToMAGolden.value) {
              const timeStr = `${currentTime.getHours()}:${currentTime.getMinutes().toString().padStart(2, '0')}`;
              console.log(
                `  🌟 GOLDEN CROSS (GROUP) [${timeStr}]: ${group.label} - MA${goldenCross.from} crossed above MA${goldenCross.to}`
              );
              ...
              if (isAlgoActive) {
                ...
                group.symbols.forEach(symbol => {
                  ...
                  console.log(
                    `        MA${goldenCross.from}: ${fromMA?.value.toFixed(4) || 'N/A'}, MA${goldenCross.to}: ${toMA?.value.toFixed(4) || 'N/A'}`
                  );

                  // 이미 골든크로스 상태인지 확인 (from이 to보다 위에 있음)
                  if (fromMA && toMA && fromMA.value > toMA.value) {
                    console.log(
                      `        ✓ ${symbol} is already in golden cross state (MA${goldenCross.from}: ${fromMA.value.toFixed(4)}% > MA${goldenCross.to}: ${toMA.value.toFixed(4)}%)`
                    );
                      ...
                      for (const underPeriod of goldenCross.under) {
                        ...
                        if (underMA) {
                          if (fromMA.value <= underMA.value) {
                            underConditionMet = false;
                            console.log(
                              `        ⚠️  Under condition failed: MA${goldenCross.from} (${fromMA.value.toFixed(4)}%) is NOT above MA${underPeriod} (${underMA.value.toFixed(4)}%)`
                            );
                            break;
                          } else {
                            console.log(
                              `        ✅ Under condition met: MA${goldenCross.from} (${fromMA.value.toFixed(4)}%) is above MA${underPeriod} (${underMA.value.toFixed(4)}%)`
                            );
                          }
                        }
                      }
                    ...
                    if (goldenCross.minSlope !== undefined) {
                      if (fromMA.slope < goldenCross.minSlope) {
                        slopeConditionMet = false;
                        console.log(
                          `        ⚠️  Slope condition failed: MA${goldenCross.from} slope (${fromMA.slope.toFixed(3)}) is below minimum (${goldenCross.minSlope})`
                        );
                      } else {
                        console.log(
                          `        ✅ Slope condition met: MA${goldenCross.from} slope (${fromMA.slope.toFixed(3)}) >= ${goldenCross.minSlope}`
                        );
                      }
                    }
          ...
          if (prevFromMADead && prevToMADead && currFromMADead && currToMADead) {
            ...
            if (prevFromMADead.value >= prevToMADead.value && currFromMADead.value < currToMADead.value) {
              const timeStr = `${currentTime.getHours()}:${currentTime.getMinutes().toString().padStart(2, '0')}`;
              console.log(
                `  ☠️  DEAD CROSS (GROUP) [${timeStr}]: ${group.label} - MA${deadCross.from} crossed below MA${deadCross.to}`
              );
        ...
        // 시계열 데이터 저장 (이전과 같은 데이터면 스킵 - 장 마감 중복 제거)
        const groupTimeSeries2 = groupTimeSeriesMap.get(group.group);
        const lastGroupEntry =
          groupTimeSeries2 && groupTimeSeries2.length > 0 ? groupTimeSeries2[groupTimeSeries2.length - 1] : null;
        const isGroupDuplicate =
          lastGroupEntry &&
          lastGroupEntry.avgChangeRate === avgChangeRate &&
          lastGroupEntry.avgVolumeStrength === avgVolumeStrength;
  ...
  console.log('='.repeat(60));
  console.log(`Initial Balance: ${account.initialBalance.toLocaleString()}원`);
    ...
    account.holdings.forEach((holding, symbol) => {
      ...
      if (symbolData) {
        ...
        if (lastQuote && lastQuote.close) {
          ...
          console.log(
            `  ${label} (${symbol}): ${holding.quantity}주 @ ${holding.avgPrice.toLocaleString()}원 → 현재 ${lastQuote.close.toLocaleString()}원 (평가손익: ${profit >= 0 ? '+' : ''}${profit.toLocaleString()}원)`
          );
        }
      }
    });
  ...
  const totalProfit = totalAssets - account.initialBalance;
  const returnRate = (totalProfit / account.initialBalance) * 100;
  ...
  console.log(
    `Total Profit: ${totalProfit >= 0 ? '+' : ''}${totalProfit.toLocaleString()}원 (${returnRate >= 0 ? '+' : ''}${returnRate.toFixed(2)}%)`
  );
  ...
  if (transactions.length > 0) {
    ...
    transactions.forEach((tx, index) => {
      ...
      if (tx.type === 'BUY') {
        console.log(
          `   ${tx.quantity}주 @ ${tx.price.toLocaleString()}원 = ${(tx.price * tx.quantity).toLocaleString()}원 (수수료: ${tx.fees.toLocaleString()}원)`
        );
      } else {
        // 매도 시 손익 표시
        const profitStr =
          tx.profit !== undefined ? `${tx.profit >= 0 ? '+' : ''}${tx.profit.toLocaleString()}원` : '0원';
        const profitRate = tx.avgBuyPrice ? (((tx.price - tx.avgBuyPrice) / tx.avgBuyPrice) * 100).toFixed(2) : '0.00';
        const profitRateStr = `${parseFloat(profitRate) >= 0 ? '+' : ''}${profitRate}%`;

        console.log(
          `   ${tx.quantity}주 @ ${tx.price.toLocaleString()}원 (매수가: ${tx.avgBuyPrice?.toLocaleString()}원)`
        );
