...
export default (w: Window) => {
  ...
  class LottoPage extends w.HTMLElement {
    ...
    private statsRounds: number = 5;

    private excludeLatestRound: boolean = false;

    private overlapAllowance: number = 10;

    private setRequestCount: number = 5;

    private includeBonusBall: boolean = true;
    ...
    private async loadStatsData(count: number) {
      ...
      targetList.forEach(item => {
        [
          ...
          item.tm5WnNo,
          item.tm6WnNo,
          ...(this.includeBonusBall ? [item.bnsWnNo] : []),
        ].forEach((num) => {
    ...
    // ---------- light DOM 컨텐츠 렌더 (@innerHtmlLight -> shadow <slot>) ----------

    @innerHtmlLight
    private renderTabContent(): string {
      const isStats = this.activeTab === 'stats';
      return `
        <style>
          ${this.contentStyles}
        </style>
        ${isStats ? this.buildStatsControls() : this.buildRoundControls()}
        <div class="lotto-paper">
          <div class="grid">${isStats ? this.buildStatsGrid() : this.buildRoundGrid()}</div>
        </div>
        ${isStats ? this.buildRecommendationArea() : ''}
      `;
    }
    ...
    private buildRoundControls(): string {
    ...
    private buildStatsControls(): string {
      return `
        <div class="controls">
          <span>최근</span>
          <input type="number" id="stats-select" value="${this.statsRounds}" min="1" max="100">
          <span>회 분석</span>
        </div>
        <div class="controls" style="justify-content: center; margin-top: -8px;">
          ...
          <label class="exclude-label">
            <input type="checkbox" id="include-bonus-checkbox" ${this.includeBonusBall ? 'checked' : ''}>
            보너스볼 포함
          </label>
        </div>
      `;
    ...
    private buildBall(n: number): string {
    ...
    private buildResults(): string {
      if (this.recommendedSets.length === 0) return '';

      const uniqueCount = Array.from(new Set(this.recommendedSets.flat())).length;

      const combineSection = `
        <div class="combine-section">
          <div class="combine-header">
            <span class="combine-label">추출된 ${uniqueCount}개 번호에서 조합</span>
            <button id="combine-btn">조합하기</button>
          </div>
          ${this.combinedSet.length === 6 ? `
            <div class="combine-result">
              <div class="ball-container">
                ${this.combinedSet.map(n => this.buildBall(n)).join('')}
              </div>
            </div>
          ` : ''}
        </div>
      ...
      const setsHtml = this.recommendedSets.map((set, idx) => `
        <div class="set-row">
          <div style="font-size: 11px; color: #888; margin-bottom: 5px;">Set ${idx + 1}</div>
          <div class="ball-container">
            ${set.map(n => this.buildBall(n)).join('')}
          </div>
        </div>
      `).join('');

      return `${combineSection}<div class="results-list">${setsHtml}</div>`;
    }

    private buildRecommendationArea(): string {
      return `
        <div class="recommendation-area">
          <div class="rec-title">분석 기반 번호 추천</div>
          <div class="controls">
            <input type="number" id="allowance-select" value="${this.overlapAllowance}" min="0" max="100">
            <span>% 중복 허용</span>
    ...
    // ---------- 이벤트 핸들러 (동적 영역은 @addEventListener delegate) ----------

    @event('.tabs', 'change', { preventDefault: true })
    async onTabChange(e: Event) {
      const form = (e.target as HTMLInputElement).form!;
      const formData = new FormData(form);
      const tab = formData.get('tab') as 'round' | 'stats';
    ...
    @addEventListener('#round-select', 'change', { delegate: true, root: 'light' })
    ...
    @addEventListener('#stats-select', 'input', { delegate: true, root: 'light' })
    async onStatsChange(e: Event) {
      const input = e.target as HTMLInputElement;
      let val = Number(input.value);
      if (val < 1) val = 1;
      if (val > 100) val = 100;
      this.statsRounds = val;
      await this.loadStatsData(this.statsRounds);
    }

    @addEventListener('#allowance-select', 'input', { delegate: true, root: 'light' })
    onAllowanceChange(e: Event) {
      const input = e.target as HTMLInputElement;
      let val = Number(input.value);
      if (val < 0) val = 0;
      if (val > 100) val = 100;
      this.overlapAllowance = val;
    }

    @addEventListener('#set-count-input', 'input', { delegate: true, root: 'light' })
    ...
    @addEventListener('#recommend-btn', 'click', { delegate: true, root: 'light' })
    onRecommendClick() {
      this.generateRecommendation();
    }

    @addEventListener('#combine-btn', 'click', { delegate: true, root: 'light' })
    onCombineClick() {
      this.generateCombination();
    }

    @addEventListener('#exclude-latest-checkbox', 'change', { delegate: true, root: 'light' })
    ...
    @addEventListener('#include-bonus-checkbox', 'change', { delegate: true, root: 'light' })
    async onIncludeBonusChange(e: Event) {
      const checkbox = e.target as HTMLInputElement;
      this.includeBonusBall = checkbox.checked;
      await this.loadStatsData(this.statsRounds);
    }
